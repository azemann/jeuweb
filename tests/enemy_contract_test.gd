...
func _run() -> void:
	...
	for archetype_id: StringName in INDUSTRIAL_ROSTER:
		...
		if roster_enemy != null:
			...
			var roster_frames := (roster_enemy.get_node_or_null("Visuals/GroundPivot/BodySprite") as AnimatedSprite2D)
			if roster_frames == null:
				roster_frames = roster_enemy.get_node_or_null("Visuals/BodySprite") as AnimatedSprite2D
	...
	if reaction_enemy != null:
		...
		_check(reaction_enemy.get_node("Visuals/GroundPivot/BodySprite").animation == &"hit", "Un dégât accepté doit jouer hit immédiatement.")
		_check(not reaction_enemy.patrol_component().movement_enabled, "La patrouille doit se suspendre pendant hit.")
		await create_timer(0.55).timeout
		_check(reaction_enemy.get_node("Visuals/GroundPivot/BodySprite").animation == &"walk", "La marche doit reprendre après hit.")
	...
	if dying_enemy != null:
		...
		_check(dying_enemy.get_node("Visuals/GroundPivot/BodySprite").animation == &"death", "Zéro PV doit jouer death.")
